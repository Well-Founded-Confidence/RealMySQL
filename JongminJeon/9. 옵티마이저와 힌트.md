...
![alt](./src/img/9.16.%20중첩%20루프%20조인과%20해시조인의%20처리%20성능.png)

- Throughput (처리량), Response-Time (응답 시간) 2가지 관점
	- Nested Loop Join이 최초 Record를 제공하는게 빠르다.

해시조인은 조인 조건 컬럼에 인덱스가 없거나, 조인 대상 테이블의 거의 모든 데이터를 조인하는 경우
- MySqL 8.0.18 에서 도입
- MySQL 8.0.20 버전 부터는 block nested loop 조인을 대체함
...
2단계로 나누어짐
1. 빌드 단계: 조인 대상 테이블 중에서 레코드 건수가 적은 테이블을 골라 해시 테이블 생성
2. 프로브 단계: 나머지 테이블의 레코드를 읽어 해시 테이블에서 일치하는 값을 찾음
...
해시 조인 과정
![alt](./src/img/9.17.%20해시조인(메모리에서%20모두%20처리).png)

메모리에서 모두 처리하는 과정 (1, 2차)
- 잘 모르겠는 부분: 빌드 테이블을 청크로 나누면 각 청크에는 일부 해시값만 있을텐데 조인이 가능할까?
- 말씀듣고 나니까 이해가 된거 같아용
![alt](./src/img/9.18.%20해시조인%201차%20처리.png)
![alt](./src/img/9.19.%20해시조인%202차%20처리.png)

#### 9.3.1.20. 인덱스 정렬 선호(prefer_ordering_index)

- 옵티마이저가 인덱스를 이용한 정렬이 가능한 실행계획을 선호하는 것
```sql
mysql> EXPLAIN
 SELECT *
 FROM employees
...
explain analyze  
 ...
 WHERE hire_date BETWEEN '1985-01-01' AND '1985-02-01'  
 ORDER BY emp_no;
-> Sort: employees.emp_no  (cost=11.5 rows=25) (actual time=0.204..0.209 rows=25 loops=1)  
    -> Index range scan on employees using ix_hiredate over ('1985-01-01' <= hire_date <= '1985-02-01'),   
    with index condition: (employees.hire_date between '1985-01-01' and '1985-02-01')  (cost=11.5 rows=25) (actual time=0.0379..0.18 rows=25 loops=1)

-- 2. Using emp_no index (PK)
-- 30만개를 읽어 25개 반환 (정렬은 안함)
explain analyze  
 SELECT *  
 FROM employees use index(`PRIMARY`)  
 WHERE hire_date BETWEEN '1985-01-01' AND '1985-02-01'  
 ...
 -> Filter: (employees.hire_date between '1985-01-01' and '1985-02-01')  (cost=30177 rows=33287) (actual time=17.7..138 rows=25 loops=1)  
    -> Index scan on employees using PRIMARY  (cost=30177 rows=299612) (actual time=0.0694..85.1 rows=300024 loops=1)
...
1. hire_date로 레인지스캔 후 emp_no로 정렬
2. emp_no를 순서대로 읽으며 hire_date를 필터링한 데이터 반환

여기서 1번을 선호하는게 인덱스 정렬 선호이다.
- hire_date 컬럼의 조건에 부합되는 레코드 건수에 따라 어떤 것이 효율적인지는 다를 것이다.
- 옵티마이저가 잘못된 선택을 할 수도 있다.
	- Order by를 위한 인덱스에 너무 가중치를 부여하지 않는 옵티마이저 옵션: `prefer_ordering_index`


### 9.3.2. 조인 최적화 알고리즘

1. Exhaustive: 모든 경우의 수 완전 탐색
2. Greedy: 여러 단계로 나눠 각 단계의 최적값 연결

#### 9.3.2.1. Exhaustive 검색 알고리즘

![alt](./src/img/9.20.%20Exhaustive%20검색%20알고리즘.png)
#### 9.3.2.2. Greedy 검색 알고리즘

![alt](./src/img/9.21.%20Greedy%20검색%20알고리즘.png)

- optimizer_search_depth 시스템 변수에 정의된 개수의 테이블로 가능한 조인 조합을 생성
	- 0 ~ 62로 세팅가능
	- 0 으로하면 옵티마이저가 알아서 선택
	- 4~5가 적절
- optimizer_prune_level
	- 0, 1
	- 1 : 휴리스틱 알고리즘 사용
		- 첫번째 조인의 비용이 100, 이후 조인들의 비용을 계산중에 100을 넘으면 중간에 계산 중지
	- 0: 휴리스틱 알고리즘 미사용
	- 휴리스틱 조인 최적화는 조인 대상 테이블이 몇개 없어도 성능에 영향을 많이 미침, 특별한 이유가 없다면 1로 두는 것을 권장

![alt](./src/img/9.22.%20optimizer_search_depth.png)

- optimizer_prune_level: 0으로 꺼놓은 상태

## 9.4. 쿼리 힌트

- 통계 정보나 옵티마이저의 최적화 방법들이 더 다양해지면서 쿼리의 실행 계획 최적화가 많이 성숙
- 하지만 여전히 MySQL 서버는 우리가 서비스하는 비즈니스를 100% 이해하지는 못함
- 따라서 서비스 개발자나 DBA보다 MySQL 서버가 부족한 실행 계획을 수립할 때가 있을수 있음
- 이런 경우에는 `옵티마이저에게 쿼리의 실행 계획을 어떻게 수립해야 할지 알려주는 방법` -> 힌트

### 9.4.1. 인덱스 힌트

- straight_join, use index 등을 포함
- MySQL에 옵티마이저 힌트가 도입되기 이전에 용했던 기능으로 ANSI-SQL 표준 문법을 준수하지 않음
- 옵티마이저 힌트는 MySQL을 제외한 다른 RDBMS에서는 주석으로 해석하기에 ANSI-SQL을 준수한다고 볼 수 있음
- 인덱스 힌트는 Select, Update 명령에서만 사용 가능

#### 9.4.1.1. Straight_join

- 조인 순서를 정함: from 절에 정의된 테이블 순서대로 조인
- 레코드 수가 적은 테이블이 드라이빙 테이블로 사용되지 않는 경우에 사용
```sql
...
-- explain analyze
-> Nested loop inner join  (cost=717860 rows=323646) (actual time=0.35..548 rows=331603 loops=1)  
    -> Nested loop inner join  (cost=361850 rows=323646) (actual time=0.216..408 rows=331603 loops=1)  
        -> Table scan on e  (cost=30177 rows=299612) (actual time=0.0397..61.8 rows=300024 loops=1)  
        -> Covering index lookup on de using ix_empno_fromdate (emp_no=e.emp_no)  (cost=0.999 rows=1.08) (actual time=854e-6..0.00106 rows=1.11 loops=300024)  
    -> Single-row index lookup on d using PRIMARY (dept_no=de.dept_no)  (cost=1 rows=1) (actual time=309e-6..329e-6 rows=1 loops=331603)
```

#### 9.4.1.2. USE INDEX / FORCE INDEX / IGNORE INDEX

- 조인의 순서를 변경하는 것 다음으로 자주 사용되는 것이 인덱스 힌트

- use index: 인덱스 사용 권장
- force index: 인덱스 사용 권장 (더 강한 느낌)
- ignore index: 인덱스 사용 X
...
```
- Tip: 전문 검색 인덱스가 있는 경우에는 전문 검색 인덱스가 많이 선택된다
	- 옵티마이저가 전문 검색 인덱스, PK에 가중치를 부여

- 가장 훌륭한 최적화는 그 쿼리를 서비스에서 없애 버리거나 튜닝할 필요가 없게 데이터를 최소화하는 것이며, 그것이 어렵다면 데이터 모델의 단순화를 통해 쿼리를 간결하게 만들고 힌트가 필요치 않게 하는 것
- 어떤 방법도 없다면 그 다음으로는 힌트를 선택하는 것인데, 일반적으로 실무에서는 앞쪽의 작업들이 상당한 시간과 작업 능력이 필요하기 때문에 거의 힌트에 의존

#### 9.4.1.3. SQL_CALC_FOUND_ROWS

- 실행한 쿼리의 전체 레코드 건수를 반환
- 페이징 기능 구현에 활용
- 성능향상보다는 개발자의 편의를 위해 만들어진 힌트
- 대부분의 경우 조회, Count 쿼리를 나누는 것이 좋음
...
인덱스 수준 힌트: 인덱스 수준의 힌트는 반드시 테이블명이 선행 되어야 함
...
쿼리블록 힌트
- 쿼리블록에 subq1 이라는 이름을 부여하고 JOIN_ORDER 힌트에 사용
...
- 옵티마이저 힌트 중에서 유일하게 쿼리의 실행계획에 영향을 미치지 않는 힌트
- MAX_EXECUTION_TIME: 밀리초 단위
```
...
![alt](./src/img/9.%20max_execution_time.png)
...
- SET_VAR 힌트는 실행 계획을 바꾸는 용도뿐만 아니라 조인 버퍼나 정렬용 버퍼(소트 버퍼)의 크기를 일시적으로 증가시켜 대용량 처리 쿼리의 성능을 향상시키는 용도로도 사용
- 다양한 형태의 시스템 변수를 조정할 수 있음 (모든 변수를 조정할 수 있는 것은 아님)
...
- 8.0.19 버전 이전까지는 Block Nested Loop 조인을 사용했지만 8.0.20 버전 부터는 8.0.18 버전에서 도입된 해시조인으로
- Hashjoin 제어를 하고 싶으면 BNL, NO_BNL 힌트를 사용해야함 (8.0.18 버전에서는 hashjoin 힌트 사용가능)
...
-> Nested loop inner join  (cost=143453 rows=323646) (actual time=0.113..949 rows=331603 loops=1)  
    -> Table scan on e  (cost=30177 rows=299612) (actual time=0.0715..72.3 rows=300024 loops=1)  
    -> Index lookup on de using ix_empno_fromdate (emp_no=e.emp_no)  (cost=0.27 rows=1.08) (actual time=0.00264..0.00282 rows=1.11 loops=300024)
```
...
-- No merge: 서브쿼리를 외부 쿼리와 병합하지 않음, 따라서 임시테이블 생성
mysql> EXPLAIN
 SELECT /*+ NO_MERGE(sub)*/ *
 FROM (SELECT *
 FROM employees
...
- Index_merge: 인덱스를 여러개 사용해서 합집합, 교집합으로 데이터 리턴
...
- 인덱스 컨디션 푸쉬다운(ICP, Index Condition Pushdown): 사용 가능하다면 항상 성능 향상에 도움됨
...
![alt](./src/img/9.%20인덱스,%20옵티마이저힌트.png)

MySQL 예전 버전에서 사용하던 인덱스 힌트를 대체하는 용도
